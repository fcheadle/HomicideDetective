...
namespace HomicideDetective.Places
{
    ...
    /// </summary>
    public class Weather : RogueLikeComponentBase
    {
        ...
        {
            (x,y,t) => Math.Sin(x + t.TotalMilliseconds / 111) + Math.Cos(y - t.TotalMilliseconds / 555), //fave
            (x,y,t) => 1.5 * Math.Sin(x / 3.33 + t.TotalSeconds) + Math.Cos(y * 3.33 + t.TotalSeconds), //fave
            (x,y,t) => Math.Sin(x + t.TotalMilliseconds / 444) + Math.Cos(y + t.TotalMilliseconds / 333),//fave
            (x,y,t) => -Math.Cos(x - t.TotalSeconds) - Math.Sin(y + t.TotalSeconds), //fave
